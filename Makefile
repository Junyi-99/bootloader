...
ifeq ($(DEBUG), 1)
  LD_FILE = linker/nrf52840_debug.ld
else
  LD_FILE = linker/nrf52840.ld
endif
...
ifeq ($(FLASHER),nrfjprog)
  ...
  FLASH_CMD = $(PYOCD) flash -t nrf52840 $1
  FLASH_NOUICR_CMD = $(PYOCD) flash -t nrf52840 $1
  FLASH_ERASE_CMD = $(PYOCD) erase -t nrf52840 --chip
else
...
C_SRC += $(NRFX_PATH)/mdk/system_nrf52840.c
...
# Assembly Files
#------------------------------------------------------------------------------
ASM_SRC = $(NRFX_PATH)/mdk/gcc_startup_nrf52840.S
